...
do
	...
	bcftools mpileup -Ob -o ${filename}.bcf -f ref/phyloA.fasta ${filename}.aln-fixmate.bam     
	bcftools call -vmO z -o ${filename}.vcf.gz ${filename}.bcf
	bcftools call -mO z -o ${filename}.allPos.vcf.gz ${filename}.bcf

	# normalize indels
	bcftools norm -f ref/phyloA.fasta ${filename}.vcf.gz -Ob -o ${filename}.norm.vcf.gz
	...
	cat ref/phyloA.fasta | bcftools consensus -i 'type="snp"' ${filename}.filtered.vcf.gz > ${filename2}.fasta
