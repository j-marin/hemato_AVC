...
ls mapping_virulence/*.sam > list.sam.txt
...
do
	...
	outdir="${filename/mapping_virulence/stats}"
...
do
	...
	bcftools mpileup -Ob -o ${filename}.bcf -f ../snippy/ref/RefSeq_virulence.fasta ${filename}.aln-fixmate.bam     
	bcftools call -vmO z -o ${filename}.vcf.gz ${filename}.bcf
	bcftools call -mO z -o ${filename}.allPos.vcf.gz ${filename}.bcf

	# normalize indels
	bcftools norm -f ../snippy/ref/RefSeq_virulence.fasta ${filename}.vcf.gz -Ob -o ${filename}.norm.vcf.gz

	# filter the data
	bcftools filter -O z -o ${filename}.filtered.vcf.gz -g 12 -i '%QUAL > 50 & MQ > 20 & DP > 20 & (DP4[2]+DP4[3])/sum(DP4) >= 0.9' ${filename}.norm.vcf.gz
    
	# transform as Fasta
	filename2="${filename/mapping_virulence/Fasta}"
	filename3="${filename/mapping_virulence/}"
	filename3="${filename3///}"
	bcftools index $filename.filtered.vcf.gz
	cat ../snippy/ref/RefSeq_virulence.fasta | bcftools consensus -i 'type="snp"' ${filename}.filtered.vcf.gz > ${filename2}.fasta
